...
当前 `azure` / `codex` / `claude` 三种 provider 已全部退化为 advisor 适配层：`--coordinator azure|codex|claude` 仅决定“顶层建议由谁生成”，不再承载 direct coordinator turn/tool/thread 壳层。当前共享控制链路已经演进为 `Working Memory -> Strategy -> Policy -> coordinator_core`，统一执行 `spawn / bump / broadcast`。
