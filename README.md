...
- `Strategy Layer`：单题策略解释层（把 runtime state 与 working memory 归纳为 `stage / goal / active_hypothesis / next_actions`）
- `Knowledge Store`：跨题可复用知识（从 working memory 晋升 + 命中与已应用保护）
- `Capability Layer`：执行面能力装配层（把 `ChallengeProfile + RuntimeProfile` 装配成 `ResolvedCapabilities`，统一产出工具暴露与 prompt 片段）
...
当前 `azure` / `codex` / `claude` 三种 provider 已全部退化为 advisor 适配层：`--coordinator azure|codex|claude` 仅决定“顶层建议由谁生成”，不再承载 direct coordinator turn/tool/thread 壳层。当前共享控制链路已经演进为 `Working Memory -> Strategy -> Policy -> coordinator_core`，而执行面则进一步收敛为 `ChallengeProfile + RuntimeProfile -> Capability Layer -> Solver Runtime`，统一执行 `spawn / bump / broadcast` 与 solver 工具装配。
...
flowchart TB
    ...
    subgraph Control["控制层（共享 coordinator loop + 显式控制面）"]
        ...
        SL["Strategy Layer<br/>stage / goal / active_hypothesis"]:::control
        ...
        CL["Capability Layer<br/>ChallengeProfile + RuntimeProfile -> ResolvedCapabilities"]:::control
...
2. 共享 `coordinator_loop` 汇总平台事实、当前 swarm/结果/成本，构建 `Runtime State` 快照，并增量提炼 `Working Memory`、归纳 `Strategy Layer`、晋升/匹配 `Knowledge Store`。
3. `Policy Engine` 基于 `Runtime State + Strategy + Memory + Knowledge` 产出结构化动作列表，并由 `coordinator_core` 执行。
4. 每道未解题由 `Swarm` 并行驱动多模型 `Solver Runtime`；solver 在容器中执行前，先由 `Capability Layer` 根据题目特征和 runtime 约束装配出统一的工具暴露与 prompt 片段。执行 trace 再回流到记忆与策略闭环。平台确认提交正确后，系统会自动收尾对应 swarm，并按配置决定是否释放环境、生成 writeup、退出整场。
...
## Coordinator Smoke 回归

如果你想把“长跑观察日志”改成可重复执行的回归检查，现在可以直接用仓库内置脚本：

```bash
uv run python scripts/run_coordinator_smoke.py --duration-seconds 90
```

这条默认命令会直接跑 README 里那条“凌虚赛事 CTF + Azure 总控 + Azure solver”组合，也就是：
...
脚本的成功判定不是“只要子进程没崩就算过”，而是至少同时命中两类日志：

- `startup`：必须看到 `Coordinator starting:`
- `activity`：必须看到 `Policy action executed:`、`Headless event:`、`Azure coordinator event:`、`Codex coordinator event:`、`Claude coordinator event:` 之一

达到持续时长后，脚本会主动向整个进程组发出 `SIGINT`，等待优雅退出；如果 marker 不全，或者子进程过早非零退出，脚本会返回非零状态，适合放进你自己的回归流水线。

如果你要替换成自己的 smoke 命令，用 `--` 把真实命令透传进去即可：
...
```bash
uv run python scripts/run_coordinator_smoke.py --duration-seconds 5 -- \
  python -c "import time; print('Coordinator starting: ok'); print('Policy action executed: ok'); time.sleep(1)"
```
