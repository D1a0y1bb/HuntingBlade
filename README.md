...
当前 `azure` / `codex` / `claude` 三种 provider 已全部退化为 advisor 适配层：`--coordinator azure|codex|claude` 仅决定“顶层建议由谁生成”，不再承载 direct coordinator turn/tool/thread 壳层。当前已统一在共享 `coordinator_loop + policy_engine + coordinator_core` 路径执行的是 `spawn / bump / broadcast`。
